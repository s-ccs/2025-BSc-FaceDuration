...
# ╔═╡ c16bf4a0-aada-11f0-1980-97191541aa85
begin
	using BSplineKit
...
# ╔═╡ bb3aaf2f-17a0-409c-9e4d-c39a209225f0
md"""
# Data Loading
"""

# ╔═╡ f9927203-4f27-45cd-80a5-d5b67e7a1cf0
begin
...
# ╔═╡ b9718e85-fb70-4ddf-b961-b624700baeb0
begin
...
# ╔═╡ 839c1318-fe68-4b7e-b79d-7f9996e4a638
data_df

# ╔═╡ 56a6b3c0-53c3-46e9-ade9-721ef9c31234
md"""
...
# ╔═╡ 7226f41d-f6e2-4a30-bf19-f83f5a789cba
begin
...
# ╔═╡ 1f2742dc-8860-4238-ae27-f962194b93ef
begin
...
# ╔═╡ 519856f6-26f8-4319-b747-37523fdac5c2
data_df[1, :events]

# ╔═╡ 2c795b86-af70-4920-a94f-d5e85ab83f02
md"""
# Visualizations
"""

# ╔═╡ 76536840-8d01-4a7d-8e60-9bed7ea4f2c5
begin
	# Collect stimulus and ISI events
	all_events = vcat([d.events for d in eachrow(data_df)]...)
	stimOnset_events = filter(:value => ==("stimOnset"), all_events)
	stimOffset_events = filter(:value => ==("stimOffset"), all_events)
end

# ╔═╡ 51177162-416c-410e-82bc-11da816f3ee2
# ╠═╡ disabled = true
#=╠═╡
hist(collect(skipmissing(stimOnset_events.duration)))
  ╠═╡ =#

# ╔═╡ cfbfbb66-89f0-4b54-95f5-b58c46181eb6
# ╠═╡ disabled = true
#=╠═╡
hist(collect(skipmissing(stimOffset_events.duration)))
  ╠═╡ =#

# ╔═╡ e6678870-595f-47a8-a953-99bc5d0b9151
begin
	# Generate plots for stimulus duration & ISI duration distribution
	# size for thesis figure: 1000, 300
	# size for presentation: 1000, 400
	
	f = Figure(size=(1000,400))
	ax1 = Axis(f[1, 1], 
			   title = "Trials per stimulus duration (face presentation)",
			   xlabel = "Duration (seconds)",
			   ylabel = "Trials count")
	ax2 = Axis(f[1, 2], 
			   title = "Trials per inter-stimulus interval (ISI)",
			   xlabel = "Duration (seconds)",
			   ylabel = "Trials count")
	
	hist!(ax1, collect(skipmissing(stimOnset_events.duration)), color=:skyblue2)
	hist!(ax2, collect(skipmissing(stimOffset_events.duration)), color=:royalblue2)
