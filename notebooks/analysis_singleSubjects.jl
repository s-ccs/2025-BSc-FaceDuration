...
# ╔═╡ f5bbe75e-a9ff-11f0-1626-971c53423d5b
begin
	using BSplineKit
...
# ╔═╡ db360f88-d113-4a14-8f67-ac9c42822603
md"""
# Data Loading
"""

# ╔═╡ 7254bbdb-0664-4f4e-83e8-e67b6523104e
begin
...
# ╔═╡ 7abe518d-bc3c-4c04-94a0-ee345fb62dd3
begin
...
# ╔═╡ 3c78c614-f7eb-4742-939d-25d2f716e5a8
data_df

# ╔═╡ bccd39c0-162f-42a5-bc06-7a6a91f6a399
md"""
...
# ╔═╡ a45eefcc-a880-4bb2-8b76-993706cdca6b
begin
...
# ╔═╡ 65a59f3f-80e1-4ea9-a43c-84a660e02bd0
begin
...
# ╔═╡ 4579bfe1-3b19-477e-a2c3-04f41598802f
data_df[1, :events]

# ╔═╡ 4466de0d-9eda-4cc5-b7b6-2e1b51d53701
md"""
# Individual subject inspection

Channels of interest: P7, P8, PO7, PO8, O1, O2

**Attention:** Subjects 22, 25 had suspect results regarding 50Hz frequency denoising

#### Selection of subject and channel for individual inspection

"""

# ╔═╡ 4b9b9453-3a7f-4e14-89b8-1cdc0ce156be
begin
	# Select subject for inspection
	subject_selection = [i => s for (i, s) in enumerate(data_df.subject)]
	@bind subject Select(subject_selection)
end

# ╔═╡ a6054f19-2c1f-42b3-a412-916497c89b04
subject_number = data_df[subject, :subject]

# ╔═╡ 64dbed04-e797-4494-a9e7-5a120714cbb6
# Select channel for inspection
@bind channel Select(["P7","P8","PO7","PO8","O1","O2"])

# ╔═╡ bf6f0b49-1ace-4257-a62a-b931030eab06
begin
...
# ╔═╡ 16f82cea-6bdc-4444-9730-da6f12c9b452
begin
	# Separate eeg data and events
	data = UnfoldBIDS.raw_to_data(data_df[subject, :raw], channels=[channel])
	events = dropmissing(data_df[subject, :events], :blank)

	# Remove bad data according to ASR rejection
	data = remove_bad_data(data, subject_number)
end

# ╔═╡ 5245cac5-3e63-4ddd-a3ee-61a2727b7fc8
begin
...
# ╔═╡ 95748b8c-a3b3-48aa-a5eb-10cf55a278c1
# Define basis function for deconvolution models
basisfunction = firbasis(τ = (-0.2,0.8), sfreq = 250)

# ╔═╡ 090adae0-de31-40b7-b0ba-29cc3e01eaab
begin
	# Epoch data for mass univariate models
	data_epochs, times = Unfold.epoch(data = data, tbl = events, τ = (-0.2, 0.8), sfreq = 250);

	size(data_epochs)
end

# ╔═╡ 33448076-6d8f-4d2b-8e1c-f931588f4be1
md"""
## ISI effect
"""

# ╔═╡ c87118b4-6bae-48dc-9c1e-035b1dcad710
# Fit Mass univariate model
m_mu = fit(UnfoldModel, ["stimOnset" => (f_blank, times)], events, data_epochs; eventcolumn="value")

# ╔═╡ a07db56a-773a-4742-aa9d-3ffe481387ac
# Visualize marginal effect of categorical variable 'blank' for mu model
begin
	blank_eff_m_mu = effects(Dict(:blank => ["true", "false"]), m_mu)
	plot_erp(blank_eff_m_mu; 
			 mapping = (; color = :blank),
			 axis = (; title = "Marginal Effect of Blank (Mass Univariate) for subject $subject_number ($channel channel)")
			)
end

# ╔═╡ faaf1fbb-cca8-457f-ab3e-e06fb73b561d
# Fit deconvolution model
m_oc = fit(UnfoldModel,["stimOnset"=>(f_blank, basisfunction)],events,cu(data); solver =(x, y) -> Unfold.solver_predefined(collect(x), collect(y); solver=:qr), eventcolumn="value")

# ╔═╡ 336785b8-a40e-4443-89cc-0f67c3540c4b
# Visualize marginal effect of categorical variable 'blank' for deconvolution model
begin
	blank_eff_m_oc = effects(Dict(:blank => ["true", "false"]), m_oc)
	plot_erp(blank_eff_m_oc; 
			 mapping = (; color = :blank),
			 axis = (; title = "Marginal Effect of Blank (Overlap Corrected) for subject $subject_number ($channel channel)"  , limits = (nothing, (-8, 8)))
			)
end

# ╔═╡ bd805e32-d865-466f-97d9-67d8b2f64f6d
md"""
## Stimulus duration effect
"""

# ╔═╡ b9394b27-b1ea-4529-a30f-2fde94216139
begin
	# Define basis funcition dictionary
	bf_dict_mu = [
				"true" => (f, times);
		   		"false" => (f, times)
			]
	
	# Fit linear mass univariate model
	m_mu_dur = fit(UnfoldModel, bf_dict_mu, events, data_epochs; eventcolumn=:blank);
end

# ╔═╡ 224836e6-ad50-42f3-91a5-722060a90501
begin
	# Define basis funcition dictionary
	bf_dict_mu_5spl = [
				"true" => (f_5spl, times);
	...
	# Fit 5-spline mass univariate model
	m_mu_dur_5spl = fit(UnfoldModel, bf_dict_mu_5spl, events, data_epochs; eventcolumn=:blank);
end

# ╔═╡ 53ae8dd8-6dde-438b-96eb-e27547ede1a5
begin
	# Define basis funcition dictionary
	bf_dict_oc = [
				"true" => (f, deepcopy(basisfunction));
		   		"false" => (f, deepcopy(basisfunction))
			]
	
	# Fit linear deconvolution model
	m_oc_dur = fit(UnfoldModel, bf_dict_oc, events, cu(data); solver =(x, y) -> Unfold.solver_predefined(collect(x), collect(y); solver=:qr), eventcolumn=:blank)
end

# ╔═╡ 18fb1ed8-c04e-4c60-bebf-5720188932f8
begin
	# Define basis funcition dictionary
	bf_dict_oc_5spl = [
				"true" => (f_5spl, deepcopy(basisfunction));
		   		"false" => (f_5spl, deepcopy(basisfunction))
			]
	
	# Fit 5-spline deconvolution model
	m_oc_dur_5spl = fit(UnfoldModel, bf_dict_oc_5spl, events, cu(data); solver =(x, y) -> Unfold.solver_predefined(collect(x), collect(y); solver=:qr), eventcolumn=:blank)
end

# ╔═╡ efa9cfc5-38c9-4e04-b796-2bb186a8bd82
# Visualize marginal effect of continuous variable 'duration' for linear mu model
begin
	dur_eff_m_mu = effects(Dict(:duration => 0.1:0.1:1.49), m_mu_dur)
	plot_erp(
	    dur_eff_m_mu;
	    mapping = (; color = :duration, group = :duration, row = :eventname),
		axis = (; title = "Marginal Effect of Duration (linear, mass univariate) for subject $subject_number ($channel channel)", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ c2636c4f-4966-4f16-8dae-bff0f8201c4b
# Visualize marginal effect of continuous variable 'duration' for non-linear mu model
begin
	dur_eff_m_mu_5spl = effects(Dict(:duration => 0.1:0.1:1.49), m_mu_dur_5spl)
	plot_erp(
	    dur_eff_m_mu_5spl;
	    mapping = (; color = :duration, group = :duration, row = :eventname),
		axis = (; title = "Marginal Effect of Duration (5-spline, mass univariate) for subject $subject_number ($channel channel)", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ 6a3cae77-a7fe-4177-b341-fb8289c7e74c
# Visualize marginal effect of continuous variable 'duration' for linear deconvolution model
begin
	dur_eff_m_oc = effects(Dict(:duration => 0.1:0.1:1.49), m_oc_dur)
	plot_erp(
	    dur_eff_m_oc;
	    mapping = (; color = :duration, group = :duration, row = :eventname),
		axis = (; title = "Marginal Effect of Duration (linear, overlap corrected) for subject $subject_number ($channel channel)", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ c483fbfc-b1b1-4b02-b65f-4b524c5097e5
# Visualize marginal effect of continuous variable 'duration' for non-linear deconvolution model
begin
	dur_eff_m_oc_5spl = effects(Dict(:duration => 0.1:0.1:1.49), m_oc_dur_5spl)
	plot_erp(
	    dur_eff_m_oc_5spl;
	    mapping = (; color = :duration, group = :duration, row = :eventname),
		axis = (; title = "Marginal Effect of Duration (5-spline, overlap corrected) for subject $subject_number ($channel channel)", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ 21b5e19b-9b3b-4a5c-8888-56168b0c6903
md"""
## Last stimulus in block effect

**Was not included in model formulas**
"""

# ╔═╡ 65955d91-32a2-4fb6-a314-6bfc56c75fbd
# ╠═╡ disabled = true
#=╠═╡
# Visualize marginal effect of categorical variable 'last' for deconvolution model
begin
	last_eff_m_oc = effects(Dict(:last => ["true", "false"], :blank => ["true", "false"]), m_oc)
	plot_erp(last_eff_m_oc; 
			 mapping = (; color = :last, layout = :blank),
			 axis = (; title = "Marginal Effect of last element in block (Overlap Corrected) for subject $subject_number ($channel channel)", limits = (nothing, (-8, 8)))
			)
end
  ╠═╡ =#

# ╔═╡ a971d4a6-63fc-44d8-b7bb-0c9bf0d0591c
# ╠═╡ disabled = true
#=╠═╡
# Visualize marginal effect of categorical variable 'last' for mu model
begin
	last_eff_m_mu = effects(Dict(:last => ["true", "false"]), m_mu)
	plot_erp(last_eff_m_mu; 
			 mapping = (; color = :last),
			 axis = (; title = "Marginal Effect of Blank (Mass Univariate) for subject $subject_number ($channel channel)" , limits = (nothing, (-8, 8)))
			)
end
  ╠═╡ =#

# ╔═╡ 2b48e27e-4575-4021-abba-4cb40e2f436b
md"""
# Habituation effect
"""

# ╔═╡ fe741fc4-ce9a-4484-9c8d-b6a8a8491f8a
# Visualize marginal effect of continuous variable 'stim_count' for linear deconvolution model
begin
	hab_eff_m_oc = effects(Dict(:stim_count => 1:1:120), m_oc_dur)
	plot_erp(
	    hab_eff_m_oc;
	    mapping = (; color = :stim_count, group = :stim_count, row = :eventname),
		axis = (; title = "Marginal Effect of Stimulus sequence (linear, overlap corrected) for subject $subject_number", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ 2752012c-a4b6-4240-ad5c-e972a8bb038f
# Visualize marginal effect of continuous variable 'stim_count' for linear mu model
begin
	hab_eff_m_mu = effects(Dict(:stim_count => 1:1:120), m_mu_dur)
	plot_erp(
	    hab_eff_m_mu;
	    mapping = (; color = :stim_count, group = :stim_count, row = :eventname),
		axis = (; title = "Marginal Effect of Stimulus sequence (linear, mass univariate) for subject $subject_number", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ ee359acf-c0a9-472c-9fca-77eb11b88d02
# Visualize marginal effect of continuous variable 'stim_count' for non-linear deconvolution model
begin
	hab_eff_m_oc_5spl = effects(Dict(:stim_count => 1:1:120), m_oc_dur_5spl)
	plot_erp(
	    hab_eff_m_oc_5spl;
	    mapping = (; color = :stim_count, group = :stim_count, row = :eventname),
		axis = (; title = "Marginal Effect of Stimulus sequence (5-spline, overlap corrected) for subject $subject_number", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ ae338208-d778-411d-a12e-48ae0c7ea8af
# Visualize marginal effect of continuous variable 'stim_count' for non-linear mu model
begin
	hab_eff_m_mu_5spl = effects(Dict(:stim_count => 1:1:120), m_mu_dur_5spl)
	plot_erp(
	    hab_eff_m_mu_5spl;
	    mapping = (; color = :stim_count, group = :stim_count, row = :eventname),
		axis = (; title = "Marginal Effect of Stimulus sequence (5-spline, mass univariate) for subject $subject_number", limits = (nothing, (-8, 8)))
)
end

# ╔═╡ c6f85214-c241-4fe2-9203-025c7bc6f3b6
md"""
# Results

Subjects 8, 25 showed faulty ERPs
"""
...
manifest_format = "2.0"
project_hash = "41b32d8562559f8138867495af3d25f84dd37f16"
...
git-tree-sha1 = "584af166a8723a977bc0a95e1992965055d76c82"
